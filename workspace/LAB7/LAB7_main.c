...
// BM: for radiant to meters, (this is also the radius of the wheel)
#define RADTOMETER  (0.5/8.414)
// BM: robot width, in meters
#define ROBOT_WIDTH (17.4/100)
...
float left_PID(void);
float right_PID(void);
void coordinate_cal(void);
__interrupt void ADCA_ISR (void);

// BM: new global variables
float LeftWheel_rad;
float RightWheel_rad;
float LeftWheel_m;
float RightWheel_m;
float uLeft = 0.0;
float uRight = 0.0;
...
float Left_E_k = 0;
float Left_I_k = 0;
float Right_E_k_prev = 0;
float Right_I_k_prev = 0;
float Left_E_k_prev = 0;
float Left_I_k_prev = 0;
//float K_p = 3.0;
//float K_i = 25.0;
float V_ref = 0.0;
...
// BM: labview controller global variables
float printLV3 = 0;
float printLV4 = 0;
float printLV5 = 0;
float printLV6 = 0;
...
// BM: new global variables for position calculating
float theta_avg = 0.0;
float theta_avg_dot = 0.0;
float x_dot = 0.0;
float y_dot = 0.0;
float x_dot_past = 0.0;
float y_dot_past = 0.0;

// BM: new global variables for wall tracking
uint16_t right_wall_track = 1;
float Kp_right = 0.0003;
float Kp_front = 0.0005;
float ref_right = 500.0;
float ref_front = 1400.0;
float track_speed = 0.25;
uint32_t distright = 0.0;
uint32_t distfront = 0.0;
uint32_t threshold_1 = 500;
uint32_t threshold_2 = 800;

float shift = 0.6;

// BM: Lab 7 exercise 1 declare
#define XKS_SIZE    22
int16_t adca0result;
int16_t adca1result;
int16_t counter = 0;
...
float WhlDiff_prev;
float turn_angle;
float turn_angle_prev;
float turn;
float errorDiff;
float errorDiff_prev;
float intDiff;
float intDiff_prev;
float vel_WhlDiff;
float vel_WhlDiff_prev;
float turnrate;
float turnrate_prev;
...
void main(void)
{
    ...
    // BM: use cpu timer2 for readying wheels, set to 4ms
...
//	EPwm5Regs.TBPRD = 5000; // BM: Set Period to 0.1ms sample. Input clock is 50MHz.
    ...
    setupSpib();
    // BM: call the function according to the requirement
    init_eQEPs();

    /********************* BM: for the epwm part *************************/
    // BM: setting EPWM2 REGs
	...
	EPwm2Regs.TBCTR = 0;
	// BM: set TBPRD registers, set to 20kHz
	EPwm2Regs.TBPRD = 2500;
	// BM: set CMPA registers
	EPwm2Regs.CMPA.bit.CMPA = 0;
	// BM: set CMPB registers for EPM2B
	EPwm2Regs.CMPB.bit.CMPB = 0;
	// BM: set AQCTLA registers
	EPwm2Regs.AQCTLA.bit.CAU = 1;
	EPwm2Regs.AQCTLA.bit.ZRO = 2;
	// BM: set AQCTLB registers for EPM2B
	...
	// BM: setting EPWM9 REGs
	// BM: setting TBCTL registers
	EPwm9Regs.TBCTL.bit.FREE_SOFT = 2;
	EPwm9Regs.TBCTL.bit.CTRMODE = 0;
	EPwm9Regs.TBCTL.bit.CLKDIV = 1; // BM: set to 1 according to the instruction
	EPwm9Regs.TBCTL.bit.PHSEN = 0;
	// BM: set TBCTR registers
	EPwm9Regs.TBCTR = 0;
	// BM: set TBPRD registers
	EPwm9Regs.TBPRD = 2500;
	// BM: set CMPA registers, not needed for exercise 4
	//  EPwm9Regs.CMPA.bit.CMPA = 0;
	// BM: set AQCTLA registers
	// BM: ignore CMPA by setting CAU to 0
	EPwm9Regs.AQCTLA.bit.CAU = 0;
	// BM: create square wave by setting ZRO to 3
...
//            serial_printf(&SerialA," St1 %ld St2 %ld\n\r",measure_status_1,measure_status_3);
            // BM: print out the wheel rotation value
            serial_printf(&SerialA,"Left Wheel rad: %.3f Right Wheel rad: %.3f\r\n", LeftWheel_rad, RightWheel_rad);
//            serial_printf(&SerialA,"Left Wheel rad/s: %.3f Right Wheel rad/s: %.3f\r\n", VLeftK, VRightK);
//            serial_printf(&SerialA,"Left Wheel m: %.3f Right Wheel m: %.3f\r\n", LeftWheel_m, RightWheel_m);
//            serial_printf(&SerialA,"Left Wheel m/s: %.3f Right Wheel m/s: %.3f\r\n", VLeftK_m, VRightK_m);

            // BM: print joystick voltages
            serial_printf(&SerialA,"joystick voltage 1: %.3f joystick voltage 2: %.3f\r\n", yk0, yk1);
...
__interrupt void SWI_isr(void) {
    ...
    if((numSWIcalls%62) == 0) { // change to the counter variable of you selected 4ms. timer
        DataToLabView.floatData[0] = Segbot_refSpeed;
        DataToLabView.floatData[1] = turnrate;
        DataToLabView.floatData[2] = 0;
        DataToLabView.floatData[3] = 0;
        DataToLabView.floatData[4] = 0;
    ...
    PosLeft[0] = LeftWheel;
    PosRight[0] = RightWheel;
    VLeftK = 100 * (PosLeft[0] - PosLeft[1]) + 0.6 * VLeftK_1;
    VRightK = 100 * (PosRight[0] - PosRight[1]) + 0.6 * VRightK_1;
    gyrorate_dot = 100 * (gyro_value - gyro_value_1) + 0.6 * gyrorate_dot_1;

    ubal = -K_1 * tilt_value - K_2 * gyro_value - K_3*((VRightK + VLeftK)/2.0) - K_4 * gyrorate_dot;


    WhlDiff = LeftWheel - RightWheel;

    // BM: cal vel_WhlDiff
    vel_WhlDiff = vel_WhlDiff_prev/3 + 500*(WhlDiff - WhlDiff_prev)/3;


    // BM: cal turn_angle
    turn_angle = turn_angle_prev +  0.002*(turnrate_prev + turnrate);


    // BM: cal errorDiff
    errorDiff = turn_angle - WhlDiff;

    // BM: cal intDiff
    intDiff = intDiff_prev + 0.002*(errorDiff_prev + errorDiff);


    // BM: cal turn
    turn = K_p*errorDiff + K_i*intDiff - K_d*vel_WhlDiff;

    // BM: guard
    if (fabs(turn) > 3.0) {
        vel_WhlDiff = vel_WhlDiff_prev;
        intDiff = intDiff_prev;
        errorDiff = errorDiff_prev;
    }
	...
	// BM: cal ForwardBackwardCommand
	eSpeed = (Segbot_refSpeed - (VLeftK + VRightK)/2.0);
	IK_eSpeed = IK_eSpeed_prev + (0.002 * (eSpeed + eSpeed_prev));
	ForwardBackwardCommand = KpSpeed*eSpeed + KiSpeed*IK_eSpeed;

	// BM: IK_eSpeed saturation
	if (fabs(ForwardBackwardCommand) > 3) {
	    IK_eSpeed = IK_eSpeed_prev;
	}

	// BM: ForwardBackwardCommand saturation
	...
	// BM: set the EPWM2A according to the instruction
	setEPWM2A(uRight);   // BM: to control right wheel
	setEPWM2B(-uLeft);   // BM: to control left wheel

	// Saving old variables
    PosLeft[1] = PosLeft[0];
    VLeftK_1 = VLeftK;
    PosRight[1] = PosRight[0];
    VRightK_1 = VRightK;
...
__interrupt void cpu_timer1_isr(void)
{
    // BM: increment the counter for song
//    song_count++;
    // BM: check counter bounding
//    if (song_count > SONG_LENGTH) {
//        song_count = 0;
//    }

    // BM: set TBPRD for EPwm9Regs
//    EPwm9Regs.TBPRD = songarray[song_count];
...
//      UARTPrint = 1;
    ...
    // BM: calling functions to check wheel rotation angles
    // BM: in rad
//    LeftWheel_rad = readEncLeft();
//    RightWheel_rad = readEncRight();
//    PosLeft[1] = PosLeft[0];
//    PosLeft[0] = LeftWheel_rad;
//    VLeftK = (PosLeft[1] - PosLeft[0]) / 0.004;
//    PosRight[1] = PosRight[0];
//    PosRight[0] = RightWheel_rad;
//    VRightK = (PosRight[1] - PosRight[0]) / 0.004;
//
//    // BM: in m
//    LeftWheel_m = LeftWheel_rad * RADTOMETER;
//    RightWheel_m = RightWheel_rad * RADTOMETER;
//    VLeftK_m = VLeftK * RADTOMETER;
//    VRightK_m = VRightK * RADTOMETER;
//
//    // BM: PID calculation
//    e_turn = VLeftK_m - VRightK_m + turn;
//    uRight = right_PID();
//    uLeft = left_PID();
//    // BM: set the EPWM2A according to the instruction
//    setEPWM2A(uRight);  // BM: to control right wheel
//    setEPWM2B(-uLeft);   // BM: to control left wheel
//
//    // BM: calculating the x, y, and bearing
//    coordinate_cal();
//
//
//    if (NewLVData == 1) {
//        NewLVData = 0;
////        V_ref = fromLVvalues[0];
////        turn = fromLVvalues[1];
////        printLV3 = fromLVvalues[2];
////        printLV4 = fromLVvalues[3];
////        printLV5 = fromLVvalues[4];
...
////        printLV7 = fromLVvalues[6];
////        printLV8 = fromLVvalues[7];
//        threshold_1 = fromLVvalues[3];
//        threshold_2 = fromLVvalues[4];
//        Kp_front = fromLVvalues[5];
//        ref_front = fromLVvalues[6];
//
//        Kp_right = fromLVvalues[7];
//        ref_right = fromLVvalues[2];
//    }
...
//        DataToLabView.floatData[2] = ref_right;
//        DataToLabView.floatData[3] = threshold_1;
//        DataToLabView.floatData[4] = threshold_2;
//        DataToLabView.floatData[5] = Kp_front;
//        DataToLabView.floatData[6] = ref_front;
//        DataToLabView.floatData[7] = Kp_right;
//        LVsenddata[0] = '*'; // header for LVdata
//        LVsenddata[1] = '$';
//        for (int i=0;i<LVNUM_TOFROM_FLOATS*4;i++) {
//            if (i%2==0) {
...
//    }
    ...
    // BM: ex 8 song turing controller
//    V_ref = track_speed;
//    note = songarray[song_count];
//    turn = (1.0 / (D4NOTE - A5NOTE) * note + (0.5 - D4NOTE / (D4NOTE - A5NOTE))) - shift;

    // BM: wall tracking controller
...
void setupSpib(void) //Call this function in main() somewhere after the DINT; line of code.
{
    ...
    DELAY_US(10);
    GpioDataRegs.GPCCLEAR.bit.GPIO66 = 1;
    SpibRegs.SPITXBUF = (0x7700 | 0x0011); // 0x7700
    while(SpibRegs.SPIFFRX.bit.RXFFST !=1);
    GpioDataRegs.GPCSET.bit.GPIO66 = 1;
    temp = SpibRegs.SPIRXBUF;
    DELAY_US(10);
    ...
    DELAY_US(10);
    GpioDataRegs.GPCCLEAR.bit.GPIO66 = 1;
    SpibRegs.SPITXBUF = (0x7B00 | 0x003A); // 0x7B00
    while(SpibRegs.SPIFFRX.bit.RXFFST !=1);
    GpioDataRegs.GPCSET.bit.GPIO66 = 1;
    temp = SpibRegs.SPIRXBUF;
    DELAY_US(10);
    ...
    DELAY_US(10);
    GpioDataRegs.GPCCLEAR.bit.GPIO66 = 1;
    SpibRegs.SPITXBUF = (0x7E00 | 0x0030); // 0x7E00
    while(SpibRegs.SPIFFRX.bit.RXFFST !=1);
    GpioDataRegs.GPCSET.bit.GPIO66 = 1;
    temp = SpibRegs.SPIRXBUF;
    DELAY_US(50);
...
float readEncRight(void) {
    ...
    // BM: change the sign of it so that the calculated values are all positive in left and right
    return (raw*(TWOPI / (400 * 30)));
}

// BM: control effort functions from lab 3
// BM: EPWM2A is used for the right wheel
void setEPWM2A(float controleffort) {
    // BM: check the bound of the controleffort first
    if (controleffort < -10) {
        controleffort = -10;
    }
    if (controleffort > 10) {
...
// BM: EPWM2B is used for the left wheel
void setEPWM2B(float controleffort) {
    // BM: check the bound of the controleffort first
    if (controleffort < -10) {
        controleffort = -10;
...
//float V_ref = 0.25;
float right_PID(void) {
    Right_E_k = V_ref - VRightK_m + Kp_turn * e_turn;
    // BM: saturation limit for uRight
    if (fabs(uRight) > 10.0) {
        Right_I_k = 0.95 * Right_I_k_prev;
    } else {
        Right_I_k = Right_I_k_prev + 0.004 * (Right_E_k + Right_E_k_prev) / 2;
    }
    Right_E_k_prev = Right_E_k;
    Right_I_k_prev = Right_I_k;
    return K_p * Right_E_k + K_i * Right_I_k;
}

float left_PID(void) {
    Left_E_k = V_ref - VLeftK_m - Kp_turn * e_turn;
    // BM: saturation limit for uLeft
    if (fabs(uLeft) > 10.0) {
        Left_I_k = 0.95 * Left_I_k_prev;
    } else {
        Left_I_k = Left_I_k_prev + 0.004 * (Left_E_k + Left_E_k_prev) / 2;
    }
    Left_E_k_prev = Left_E_k;
    Left_I_k_prev = Left_I_k;
    return K_p * Left_E_k + K_i * Left_I_k;
}

// BM: PosLeft and PosRight contains angles and past angles
// BM: we have theta_avg and theta_avg_past
// BM: the radius: use RADTOMETER
// BM: robot width: ROBOT_WIDTH
// BM: we have x_past and y_past
void coordinate_cal(void) {
    bearing = RADTOMETER / ROBOT_WIDTH * (PosRight[0] - PosLeft[0]);
//    theta_avg = 0.5 * (PosRight[0] + PosLeft[0]);
    theta_avg_dot = 0.5 * (VRightK + VLeftK);
    x_dot = RADTOMETER * theta_avg_dot * cos(bearing);
    y_dot = RADTOMETER * theta_avg_dot * sin(bearing);
    x += 0.004 * (x_dot + x_dot_past) / 2;
    y += 0.004 * (y_dot + y_dot_past) / 2;
    x_dot_past = x_dot;
    y_dot_past = y_dot;
}
