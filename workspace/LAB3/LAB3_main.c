...
/* BM: what we learned:
 * 1. we learned how to play with PWM signals with registers
 * 2. we learned how to control motors using PWM with GPIO pins
 * 3. we learned how to control buzzer using PWM with GPIO pins
 * 4. we learned how to control SERVO motors using PWM with GPIO pins
 * EX. we learned how to never give up using PWM with GPIO pins
 * */

// BM: never gonna give you up
#define song_len 31
uint16_t song[SONG_LENGTH] = {
                              D4NOTE,
                              E4NOTE,
                              G4NOTE,
                              E4NOTE,
...
//    ConfigCpuTimer(&CpuTimer1, LAUNCHPAD_CPU_FREQUENCY, 125000);    // BM: for execise 4 original
    ConfigCpuTimer(&CpuTimer1, LAUNCHPAD_CPU_FREQUENCY, 200000);    // BM: for execise 4 our own
...
//	EPwm9Regs.CMPA.bit.CMPA = 0;
	...
	// BM: commented out after Exercise 3
...
__interrupt void cpu_timer1_isr(void)
{
    ...
    if (song_count > song_len) {
        song_count = 0;
    }

    // BM: set TBPRD for EPwm9Regs
    EPwm9Regs.TBPRD = song[song_count];
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    // BM: adding new codes to timer2 ISR, for EPWM2A and B
    if (count == 1) {
        control_count -= 0.001;
    } else {
        control_count += 0.001;
    ...
    if (90.0 <= control_angle) {
        angle_count = 1;
    }
    if (control_angle <= -90.0) {
        angle_count = 0;
    }
	...
	// BM: setting EPWM2A CMPA and EPWM2B CMPB
	setEPWM2A(control_count);
	setEPWM2B(control_count);

	// BM: setting EPWM8A CMPA and EPWM8B CMPB
	setEPWM8A_RCServo(control_angle);
	setEPWM8B_RCServo(control_angle);
}
